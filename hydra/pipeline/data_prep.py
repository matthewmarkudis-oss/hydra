...
def _prepare_historical(
    ...
    for ticker in tickers:
        # Fetch all bars for this ticker
        daily_frames = []
        for trade_date in trading_dates:
            df = adapter.get_intraday_bars(ticker, trade_date)
    ...
    total_bars = min(len(list(all_ohlcv.values())[0]["close"]) for _ in [1]) if all_ohlcv else 0
    timestamps = np.arange(total_bars, dtype=np.int32)
