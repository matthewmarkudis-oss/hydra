...
class DailyEnvConfig(BaseModel):
    ...
    max_daily_loss_pct: float = Field(default=0.50, gt=0, le=1.0)
    normalize_obs: bool = Field(default=True)
...
class RewardConfig(BaseModel):
    ...
    pnl_bonus_weight: float = Field(default=25.0, ge=0, description="Weight for direct P&L return bonus")
    reward_scale: float = Field(default=100.0, gt=0, description="Multiplier for reward signal magnitude")
    cash_drag_penalty: float = Field(default=0.02, ge=0, description="Penalty for undeployed capital; low to avoid drowning P&L signal")
